...
from omni.isaac.lab_assets.universal_robots import UR5_CFG, UR10_CFG
...
@configclass
class UR5BaseEnvCfg(DirectRLEnvCfg):
    decimation = 4
    episode_length_s = 10.0
    action_scale = 1.0
    action_space = 6+1 # (x, y, z, roll, pitch, yaw) + gripper
    state_space = 0

    use_visual_obs = True
    use_visual_marker = True
    observation_space = [3, 64, 64] if use_visual_obs else 25
    ...
    # robot
    robot = UR5_CFG.replace(prim_path="/World/envs/env_.*/Robot")
    ...
    if use_visual_obs:
        camera = CameraCfg(
            ...
            data_types=["rgb"], 
            width=observation_space[1], height=observation_space[2],  
        )
...
class UR5BaseEnv(DirectRLEnv):
    cfg: UR5BaseEnvCfg

    def __init__(self, cfg: UR5BaseEnvCfg, render_mode: str | None = None, **kwargs):
        super().__init__(cfg, render_mode, **kwargs)
        self.action_scale = self.cfg.action_scale
        self.dt = self.cfg.sim.dt * self.cfg.decimation

        # Body and Joint ids querying
        self.joint_ids, joint_names = self.robot.find_joints([".*"], preserve_order = True) 
         # ['shoulder_pan_joint', 'shoulder_lift_joint', 'elbow_joint', 'wrist_1_joint', 'wrist_2_joint', 'wrist_3_joint', 'finger_joint', 'left_inner_knuckle_joint', 'right_inner_knuckle_joint', 'right_outer_knuckle_joint', 'left_inner_finger_joint', 'right_inner_finger_joint']
        self.body_ids, body_names = self.robot.find_bodies(['.*'], preserve_order = True) 
        # ['base_link', 'shoulder_link', 'upper_arm_link', 'forearm_link', 'wrist_1_link', 'wrist_2_link', 'wrist_3_link', 'flange', 'tool0', 'robotiq_base_link', 'gripper_center', 'left_outer_knuckle', 'left_inner_knuckle', 'right_inner_knuckle', 'right_outer_knuckle', 'left_outer_finger', 'right_outer_finger', 'left_inner_finger', 'right_inner_finger', 'left_inner_finger_pad', 'right_inner_finger_pad']
        ...
        self.robot_entity_cfg = SceneEntityCfg("robot", joint_names=['shoulder_pan_joint', 'shoulder_lift_joint', 'elbow_joint', 'wrist_1_joint', 'wrist_2_joint', 'wrist_3_joint'], body_names=["flange"])
        ...
        self.ee_idx = self.robot_entity_cfg.body_ids[0] # 'flange'
        self.finger_idx, _ = self.robot.find_bodies(['left_inner_finger_pad', 'right_inner_finger_pad'], preserve_order = True)
        # self.gripper_joint_ids, _ = self.robot.find_joints(['finger_joint', 'right_outer_knuckle_joint'], preserve_order = True)
        self.undesired_contact_body_ids, _ = self.robot.find_bodies(['base_link', 'shoulder_link', 'upper_arm_link', 'forearm_link', 'wrist_1_link', 'wrist_2_link', 'wrist_3_link'])
